...
{ 
    ...
    public static GroupElementPreComp[] ge_Bi = new GroupElementPreComp[]
        {
    ...
    public struct ec_point_pair
    {
        public byte[] a, b;

        public ec_point_pair()
        {
            a = new byte[32];
            b = new byte[32];
        }
    ...
    public struct rs_comm
    {
        public byte[] h;
        public ec_point_pair[] ab;

        public rs_comm(int size)
        {
            //h = new byte[32];
            ab = new ec_point_pair[size];
            for (int i = 0; i < size; i++)
            {
                ab[i] = new ec_point_pair(); 
            }
        }
    ...
    public struct signature
    {
        ...
        public signature(byte[] c, byte[] r)
        {
            this.c = c;
            this.r = r;
        }
    ...
    public struct RingSignature
    {
        ...
        public RingSignature(signature[] signatures)
        {
            sigs = signatures;
        }
        ...
        public static RingSignature FromByteArray(byte[] buffer)
        {
            ...
            while (offset < buffer.Length)
            {
                byte[] c = new byte[32];
                byte[] r = new byte[32];
                Array.Copy(buffer, offset, c, 0, 32);
                Array.Copy(buffer, offset + 32, r, 0, 32);
                signatures.Add(new signature(c, r));
                offset += 64;
            }
            return new RingSignature(signatures.ToArray());
        }
    }


    private static void sc_0(byte[] s)
    {
        int i;
        for (i = 0; i < 32; i++)
        {
